...
namespace gazebo
{
  ...
  private:
    ...
    // 이거는 뭔가 gazebo_mavlink_interface에서 send하는 함수들?
    ...
    // 센서의 방향 정보를 바탕으로 MAVLink 메시지에서 센서의 방향을 설정하는 함수입니다. 센서의 X축 방향을 입력받아 이를 MAVLink 메시지에 반영합니다.
    ...
    // Gazebo 시뮬레이터에서 여러 센서에 대해 구독을 생성하고, 해당 데이터를 처리하는 헬퍼 함수입니다. 센서의 위치, 방향을 설정하고 구독된 센서 데이터에 대한 처리 방식을 지정합니다.
